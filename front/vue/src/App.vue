  ...
  <v-app :dark="dark" v-if="ready">
    ...
    <v-toolbar app :clipped-left="clipped" color="primary">
      ...
      <v-menu offset-y>
        ...
        <v-list>
          <v-list-tile v-for="(display,key) of languages" :key="key" @click="setLanguage(key)" v-if="current() !== key">
    ...
    <v-snackbar v-model="snackbar.display" :color="snackbar.color" :timeout="snackbar.timeout">
      {{snackbar.text}}
      <v-btn dark flat @click="snackbar.display = false">X</v-btn>
    </v-snackbar>
  </v-app>
</template>
...
<script>
  ...
  export default {
    ...
    mounted() {
      ...
      let language = languageFromCookies || 'en'
