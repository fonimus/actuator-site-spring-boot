    ...
    <template slot="content">
      ...
      <v-data-table :headers="headers" :items="rows" :search="search" :loading="loading"
        ...
        <template slot="items" slot-scope="props">
          ...
          <td>
            <v-btn small round :color="color(props.item.effectiveLevel)">{{ props.item.effectiveLevel }}</v-btn>
          </td>
          <td>
...
<script>
  ...
  export default {
    ...
    methods: {
      ...
      updateLevel: function (name, level) {
    ...
    computed: {
      rows: function () {
        ...
        return this.tableData.filter(value => this.levelFilters.indexOf(value.configuredLevel) > -1 || this.levelFilters.indexOf(value.effectiveLevel) > -1)
      },
      headers: function () {
        return [
