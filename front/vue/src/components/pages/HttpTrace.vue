    ...
    <template slot="content">
      ...
      <v-data-table :headers="headers" :items="rows" :search="search" :loading="loading"
        ...
        <template slot="items" slot-scope="props">
          ...
          <td>
            <v-btn small round :color="color(props.item.response.status)">{{ props.item.response.status }}</v-btn>
          </td>
          <td>{{ props.item.timeTaken }}</td>
          <td>
            <v-btn small round :color="color(props.item.request.method)">{{ props.item.request.method}}</v-btn>
          </td>
...
<script>
  ...
  function status(allowed, status) {
  ...
  function method(allowed, method) {
    return !allowed || allowed.length === 0 || allowed.indexOf(method) > -1
  }
  ...
  export default {
    ...
    computed: {
      rows: function () {
        ...
        return this.tableData.filter(value => method(this.methodFilters, value.request.method) && status(this.statusFilters, value.response.status))
      },
      headers: function () {
        return [
